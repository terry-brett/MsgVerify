...
# 1. Load the dataset
df = pd.read_csv('../datasets/phishing_messages_datasets/annotated with reasoning labels/v2/twitter_spam_annotated.csv')

# 2. Filter to only spam messages (class == 1)
...
for index, row in df.iterrows():
    ...
    reasoner = MessageReasoningLabels(row['tweets'], "@handle")

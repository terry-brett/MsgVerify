...
# 1. Load the dataset
df = pd.read_csv('../datasets/phishing_messages_datasets/annotated with reasoning labels/v2/TREC_06_annotated.csv')

# 2. Filter to only spam messages (label == 1)
...
for index, row in df.iterrows():
    ...
    reasoner = MessageReasoningLabels(str(row['subject']) + "\n" + str(row['body']), str(row['sender']))
